...
type Rubros = { capital: number; utilidad: number; iva: number; repuestoExterno: number };



function distribuirRubrosProporcional(pagos: { monto: number }[], rubros: Rubros): Rubros[] {

  const total = pagos.reduce((acc, p) => acc + p.monto, 0);

  if (total <= 0) {

    return pagos.map(() => ({ capital: 0, utilidad: 0, iva: 0, repuestoExterno: 0 }));

  }



  const asignados: Rubros[] = [];

  let acumulado = { capital: 0, utilidad: 0, iva: 0, repuestoExterno: 0 };



  pagos.forEach((pago, idx) => {

    const esUltimo = idx === pagos.length - 1;

    const ratio = pago.monto / total;



    const capital = esUltimo ? round2(rubros.capital - acumulado.capital) : round2(rubros.capital * ratio);

    const utilidad = esUltimo ? round2(rubros.utilidad - acumulado.utilidad) : round2(rubros.utilidad * ratio);

    const iva = esUltimo ? round2(rubros.iva - acumulado.iva) : round2(rubros.iva * ratio);

    const repuestoExterno = esUltimo

      ? round2(rubros.repuestoExterno - acumulado.repuestoExterno)

      : round2(rubros.repuestoExterno * ratio);



    asignados.push({ capital, utilidad, iva, repuestoExterno });



    acumulado = {

      capital: acumulado.capital + capital,

      utilidad: acumulado.utilidad + utilidad,

      iva: acumulado.iva + iva,

      repuestoExterno: acumulado.repuestoExterno + repuestoExterno,
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (metodoPago === "mixto") {
      ...
      const asignaciones = distribuirRubrosProporcional(pagosNormalizados, {
      ...
      const pagosConAsignacion = pagosNormalizados.map((pago, idx) => ({

        ...pago,

        asignacion: asignaciones[idx],

      }));



      const existePendiente = pagosConAsignacion.some((p) => p.meta.estado === "Pendiente");
      ...
      const pagoRecords = pagosConAsignacion.map((pago) => {
        ...
        const montoAsignado = round2(

          pago.asignacion.capital + pago.asignacion.utilidad + pago.asignacion.iva + pago.asignacion.repuestoExterno
        ...
        return {
          ...
          "Capital Asignado": pago.asignacion.capital,

          "Utilidad Asignada": pago.asignacion.utilidad,

          "IVA Asignado": pago.asignacion.iva,

          "Repuesto Externo Asignado": pago.asignacion.repuestoExterno,
      ...
      const componentTxFields = pagosConAsignacion
        ...
        .map((pago) => {
          ...
          return {
            ...
            "Monto Total": montoAsignado,

            Capital: pago.asignacion.capital,

            Utilidad: pago.asignacion.utilidad,

            IVA: pago.asignacion.iva,

            "Repuesto Proveedor Externo": pago.asignacion.repuestoExterno,
      ...
      if (tablePendientes) {
        ...
        const pendientes = pagosConAsignacion
    ...
    if (esPendiente) {
      ...
      const pendiente = {
        ...
        "Utilidad Asignada": utilidad,
        "IVA Asignado": iva,
        "Repuesto Externo Asignado": repuestoExterno,
